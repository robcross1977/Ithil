...
public class ResponseTransformPipelineTests
{
    ...
    public async Task CallsScrubber_OnResponseBody()
    {
        ...
        _privacyFilter.ScrubAsync(Arg.Any<Stream>(), Arg.Any<CancellationToken>()).Returns("response body");
    ...
    public async Task RecordsUsage_AfterScrub()
    {
        ...
        await _budgetEngine.Received(1).RecordUsageAsync("agent-1", Arg.Any<int>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task ResponseTransformPipeline_UsesTokenCounter_NotWordSplit()
    {
        ...
        _privacyFilter.ScrubAsync(Arg.Any<Stream>(), Arg.Any<CancellationToken>()).Returns("this has four words");
        _tokenCounter.CountTokens(Arg.Any<string>()).Returns(99);

        var pipeline = CreatePipeline();
        await pipeline.TransformAsync("agent-1", "trace-abc-123", "GetInventory", body, null);

        await _budgetEngine.Received(1).RecordUsageAsync("agent-1", 99, Arg.Any<CancellationToken>());
    }
    ...
    public async Task ResponseTransformPipeline_PassesScrubbedBody_ToTokenCounter()
    {
        ...
        _privacyFilter.ScrubAsync(Arg.Any<Stream>(), Arg.Any<CancellationToken>()).Returns("scrubbed body");
