...
public class JwtIdentityResolverTests
{
    ...
    public async Task ReturnsAgentId_ForValidToken()
    {
        ...
        var result = await new JwtIdentityResolver(ValidParams).TryResolveAsync(
            ContextWithBearer(token)
        );
    ...
    {
        SymmetricSecurityKey wrongKey = new(
            Encoding.UTF8.GetBytes("wrong-signing-key-must-be-32-bytes!!")
        );
